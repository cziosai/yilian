...
public:
	...
	// 接收最后刷新时间
	time_t lastInPutRefreshTime;
	// 发送最后刷新时间
	time_t lastOutPutRefreshTime;
	// 上传流量 
	int upDataBytes;
	// 下载流星 
	int downDataBytes;
	...
	Socket()
	{
		...
		lastInPutRefreshTime = std::time(NULL);
		lastOutPutRefreshTime = std::time(NULL);
	}
	...
	Socket(long ip, int port, int protocol)
	{
		closed = false;
		upDataBytes = 0;
		downDataBytes = 0;
	...
	int dataInPut(char *buf, int size)
	{
		int ret = -1;
#ifdef WIN32
		...
		{
			lastInPutRefreshTime = std::time(NULL);
			this->upDataBytes += ret;
		}
		return ret;
	}

	int dataOutPut(char *buf, int size)
	{
		int ret = -1;
#ifdef WIN32
...
private:
	...
	int init()
	{
		...
		connect(socket_fd, (const struct sockaddr *)&sockaddr, sizeof(sockaddr));
