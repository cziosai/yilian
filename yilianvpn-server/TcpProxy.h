...
public:
    ...
    void processPacket(char *packet, int size)
    {
        ...
        else if (flags == (TCPHeader::ACK | TCPHeader::FIN))
        {
        	processACKPacket();
            processFINPacket();
        }
        ...
        else if (flags == (TCPHeader::ACK | TCPHeader::PSH | TCPHeader::URG))
        {
        	processPSHPacket();
            processACKPacket();
            processURGPacket();
        }
        else if (flags == (TCPHeader::ACK | TCPHeader::FIN | TCPHeader::PSH | TCPHeader::URG))
        {
        	processPSHPacket();
            processACKPacket();
            processURGPacket();
