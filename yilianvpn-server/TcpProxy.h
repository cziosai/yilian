...
public:
	...
	void waitConnected()
	{
		...
		int ret = pthread_create(&tid, NULL, TcpProxy::threadJob, this);
		if (ret != 0)
		{
			std::cout << "pthread_create error: error_code=" << ret << std::endl;
			close("pthread_create error");
		}
	}
	 ...
	 * 线程的运行函数
	 */
	static void* threadJob(void* args)
	{	
		TcpProxy* tcpProxy = (TcpProxy*)args;
		bool cnt = tcpProxy->isConnected(Config::TCP_CONNECT_TIMEOUT);
		if(cnt)
		{
			tcpProxy->connected = true;	
		}else
		{
			tcpProxy->close("connection init error");
		}
		//pthread_exit(NULL);
		return args;
	}
	
	bool isConnected(int timeout_sec)
	{
		int ret = true;
		...
		{
			// printf("[TcpProxy](%s) connection init error, can not connect.\n", toString().c_str());
			ret = false;	
		}
...
private:
	...
	static const int SYN_WAIT = 4;
	static const int SYN_WAIT_ACK = 5;
	static const int ESTABLISHED = 6;
	static const int CLOSE_WAIT = 7;
	static const int LAST_ACK = 8;
