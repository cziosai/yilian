...
public:
	...
	TcpProxy(long clientId, Socket *clientSocket, char *packet) : Proxy(clientId, clientSocket)
	{
		...
		tcpHeader.setFlag(0);
		tcpHeader.setWindow(TCP_WINDOW);
		tcpHeader.setUrp(0);
	...
	bool isExpire()
	{
		...
		return (now - destSocket.lastInPutRefreshTime) > Config::TCP_PROXY_EXPIRE_TIME || (now - destSocket.lastOutPutRefreshTime) > Config::TCP_PROXY_EXPIRE_TIME;
	}
