...
int main(int argc, char **argv)
{
	...
	std::cout << "[Main]program speed=" << speed << std::endl;
	App app;

	float count = 0;
	struct taskInfo info = {1, 1, 1};
	// 启动任务
	while(info.size > 0)
	{
		info = task_loop();
		...
		{
			count = 0;
			if(info.sleeped > (info.size * 0.67))
			{
			#ifdef _WIN32
				Sleep(1);
			#else
...
int setConfig(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if(argi == "help" || argi == "-h")
		{
			...
			std::cout << "mute=[arg]" << std::endl;
			std::cout << "  -m=[arg] set mute" << std::endl;
	...
	for (std::map <std::string, std::string>::iterator it = argsMap.begin(); it != argsMap.end(); ++it) {
		...
		else if(it->first == "mute" || it->first == "-m")
		{
			Config::MUTE = atoi(it->second.c_str());
		}
