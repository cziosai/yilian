...
public:
	// 控制包代码 
	static const unsigned char CTRL = 203;
	long clientId;
	...
	Client(Socket socket)
	{
		...
		authenticated = false;
		remoteSocket = socket;
		cacheBytes = new char[Config::MTU * 2];
		cacheBytesSize = 0;
		buffer = new char[Config::MTU];
	}
	...
	void sendToClient(char* bytes, int size)
	{
		int res = remoteSocket.dataOutPut(bytes, size);
		if(res == 0 || (res > 0 && res < size))
		{
			printf("[Client]client(%ld) send packet to client error, res=%d.\n", clientId, res);
			close();
		}
	}
	...
	* 接收数据包 建立新代理
	*/
	void onTCPUDPPacketReceived(char *packet, int size, unsigned char protocol)
	{
	 ...
	 * 处理ICMP包 
	 */
	void onIPCMPacketReceived(char* bytes, int size) {
        IPHeader ipHeader = IPHeader(bytes, 0);
        ICMPHeader icmpHeader = ICMPHeader(bytes, IPHeader::IP4_HEADER_SIZE);
	 ...
	 * 处理控制包 
	 */
	int onCTRLPacketReceived(char *packet, int size)
	{
		int res = 0;
	...
	int processIPPacket(char *packet, int size)
	{
		...
		unsigned char protocol = header.getProtocol();
		...
		if(!authenticated)
		{
			...
			{
				onCTRLPacketReceived(packet, size);
			}
			else
			{
		...
		else
		{
			if (protocol == IPHeader::TCP)
			{
				onTCPUDPPacketReceived(packet, size, IPHeader::TCP);
			}
			else if(protocol == IPHeader::UDP)
			{
				onTCPUDPPacketReceived(packet, size, IPHeader::UDP);
			}
			else if(protocol == IPHeader::ICMP)
			{
				onIPCMPacketReceived(packet, size);
			}
			else if(protocol == Client::CTRL)
			{
				return onCTRLPacketReceived(packet, size);
			}
			else
			{
	...
	int processRecvBytes(char *bytes, int size)
	{
		...
		if(totalLength <= 0 || totalLength > Config::MTU)
		{
	...
	bool loop()
	{
		int size = remoteSocket.dataInPut(buffer, Config::MTU);
		if (size > 0)
		{
	...
	bool isExpire()
	{
		...
		return (now - remoteSocket.lastInPutRefreshTime) > Config::CLIENT_EXPIRE_TIME || (now - remoteSocket.lastOutPutRefreshTime) > Config::CLIENT_EXPIRE_TIME;
	}
