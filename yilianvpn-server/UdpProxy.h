...
public:
	...
	UdpProxy(long clientId, Socket *clientSocket, char *packet) : Proxy(clientId, clientSocket)
	{
		...
		dataBuffer = buffer + Proxy::IP_UDP_HEADER_SIZE;
		IPHeader comeIpHeader = IPHeader(packet, 0);
		srcIp = comeIpHeader.getSourceIP();
		destIp = comeIpHeader.getDestinationIP();
		UDPHeader comeUDPHeader = UDPHeader(packet, comeIpHeader.getHeaderLength());
		srcPort = comeUDPHeader.getSourcePort();
		destPort = comeUDPHeader.getDestinationPort();
	...
	{
		// 从服务器接收数据
		int size = destSocket.dataInPut(dataBuffer, Proxy::UDP_BUFFER_SIZE);
		if (size > 0)
		{
			updateUDPBuffer(size);
			// 转发给客户端
			sendToClient(buffer, Proxy::IP_UDP_HEADER_SIZE + size);
			this->loopCount++; 
		}
		else if(size == 0)
		{
	...
	bool isExpire()
	{
		...
		return (now - destSocket.lastInPutRefreshTime) > Config::UDP_PROXY_EXPIRE_TIME || (now - destSocket.lastOutPutRefreshTime) > Config::UDP_PROXY_EXPIRE_TIME;
	}
	...
	bool equal(char *packet)
	{
		...
		return protocol == ipHeader.getProtocol() && srcIp == ipHeader.getSourceIP() && destIp == ipHeader.getDestinationIP() && srcPort == udpHeader.getSourcePort() && destPort == udpHeader.getDestinationPort();
	}

	/*
	 * 更新UDP头部
	 */
	void updateUDPBuffer(unsigned int size)
	{
		IPHeader ipHeader = IPHeader(buffer, 0);
		UDPHeader udpHeader = UDPHeader(buffer, IPHeader::IP4_HEADER_SIZE);
		ipHeader.setTotalLength(Proxy::IP_UDP_HEADER_SIZE + size);
