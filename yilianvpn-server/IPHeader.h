...
class IPHeader
{
	 ...
	 **/
public:
	static const unsigned char IP4_HEADER_SIZE = 20;
	static const unsigned char V4_VERSION = 4;
	static const unsigned char ICMP = 1;
	static const unsigned char IGMP = 2;
	static const unsigned char TCP = 6;  // 6: TCP协议号
	static const unsigned char UDP = 17; // 17: UDP协议号
	static const unsigned char offset_proto = 9; // 9：8位协议偏移
	static const unsigned char offset_src_ip = 12; // 12：源ip地址偏移
	static const unsigned char offset_dest_ip = 16; // 16：目标ip地址偏移
	static const unsigned char offset_ver_ihl = 0; // 0: 版本号（4bits） + 首部长度（4bits）
	static const unsigned char offset_tos = 1; // 1：服务类型偏移
	static const unsigned char offset_tlen = 2; // 2：总长度偏移
	static const unsigned char offset_identification = 4; // 4：16位标识符偏移
	static const unsigned char offset_flags_fo = 6; // 6：标志（3bits）+ 片偏移（13bits）
	static const unsigned char offset_ttl = 8; // 8：生存时间偏移
	static const unsigned char offset_crc = 10; // 10：首部校验和偏移
	static const unsigned char offset_op_pad = 20; // 20：选项 + 填充
	...
	{
		return (mData[mOffset + offset_ver_ihl] >> 4) & 0xF0;
	}
	...
	//计算校验和
	unsigned short checksum(unsigned long sum, char *buf, int offset, int len)
	{
	...
	unsigned long getsum(char *buf, int offset, int len)
	{
		unsigned long sum = 0;
		while (len > 1)
		{
	...
	{
		unsigned short oldCrc = getCrc();
		setCrc((unsigned short) 0);
		unsigned short newCrc = checksum(0, mData, mOffset, getHeaderLength());
		setCrc(newCrc);
