...
public:
	...
	{
    	unsigned char lenres = (unsigned char)mData[mOffset + offset_lenres];
    	return (lenres >> 4) * 4;
	}

	void setHeaderLength(unsigned char value)
	{
		unsigned char temp = mData[mOffset + offset_lenres];
		mData[mOffset + offset_lenres] = (unsigned char)((((value + 3) / 4) << 4) | (temp & 0x0F));
	}
	...
	{
    	unsigned char temp = mData[mOffset + offset_flag];
